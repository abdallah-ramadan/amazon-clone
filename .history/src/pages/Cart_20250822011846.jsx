    ...
    return (
      <div className="tw-flex tw-flex-col tw-items-center tw-justify-center tw-h-screen-minus-160 tw-text-center tw-p-4">
        ...
        <div className="tw-flex tw-gap-3">
          <button onClick={navigate("/")} className="tw-bg-yellow-400 hover:tw-bg-yellow-500 tw-text-gray-800 tw-font-medium tw-px-6 tw-py-2 tw-rounded tw-shadow tw-transition tw-cursor-pointer">
            Continue Shopping
          </button>
        </div>
      </div>
          ...
          <div className="tw-bg-white tw-pt-5 tw-pb-4">
            <div className="tw-px-5">
              ...
              <div className="tw-border-t tw-border-[#E7E7E7]">
                {cart.map((item, index) => (
                  <ProductCartItem key={index} product={item}   onUpdateCart={(updatedCart) => setCart(updatedCart)} 
 />
                ))}
              </div>
            </div>
